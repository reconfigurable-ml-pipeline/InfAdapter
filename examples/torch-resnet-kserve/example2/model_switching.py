...
def switch_model(namespace: str, service_name: str, target_port: int, new_model: str):
    ...
    async def request_pod_to_switch(session, endpoint):
        async with session.post(
                f"{endpoint}/v3/models/{service_name}:switch-model", data={"model": new_model}
        ) as response:
    ...
    update_configmap(configmap_name, namespace=namespace, data={"active_model": new_model}, partial=True)
